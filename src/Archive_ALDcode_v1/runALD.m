...
lam0 = 1;  % Initial ratio of nsevar to prior var (ie, nsevar*alpha)
% ovsc: overall scale, nasevar: noise variance
[kRidge, ovsc ,nsevar]  =  runRidge(lam0, datastruct, opts0);

%% 1. ALDs
baseOpts = optimset( 'gradobj', 'on',...
   'tolfun',1e-8, 'TolX', 1e-8, 'TolCon', 1e-8,...
   'MaxIter', 10e3, 'MaxFunEval', 30*1e3, ...
   'UseParallel','always', 'Display','off', 'Algorithm', 'trust-region-reflective');
% options: ALDs uses trust-region algorithm with analytic gradients and Hessians
opts1 = optimset(baseOpts,  'hessian', 'on');
...
opts2 = optimset(baseOpts, 'algorithm','active-set');
