function [colormap]=cbrewer2(varargin)
% [colormap]=cbrewer2(ctype, cname, ncol, interp_method)
% [colormap]=cbrewer2(cname, ncol, interp_method)
%
...
% infer ctype from cname
ctype_names= fieldnames(colorbrewer);
for ct = 1:length(ctype_names)
   if any(strcmpi(cname, fieldnames(colorbrewer.(ctype_names{ct}))))
      ctype = ctype_names{ct};
      break
   end
end
% assert(~isempty(ctype), 'map name %s did not match any known map', cname)
