...
% '-clean': call epsclean before converting
...
pdfCmd=	[epsCmd cmdEnd '-dPDFSETTINGS=/prepress'];							% Recommended by gs.
...
for n= 1:length(varargin)											% Read options.
    if(ischar(varargin{n}))
        ...
        elseif(strcmpi(varargin{n}, '-clean')),	CLEAN_EPS = true;
